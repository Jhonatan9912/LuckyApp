...
class Game(db.Model):
    ...
    user_id = Column(Integer, ForeignKey('users.id'), nullable=True)  # quién abrió el juego
    played_at = Column(DateTime, server_default=func.now())

    # ---- CAMPOS QUE USAS EN EL SERVICIO ----
    state_id = Column(Integer, nullable=False, server_default='1')   # 1=en juego, 2=finalizado
    winning_number = Column(Integer, nullable=True)

    # 👇 NUEVO: debe existir para que Game(..., digits=digits) funcione
    digits = Column(SmallInteger, nullable=False, server_default='3')
...
class GameNumber(db.Model):
    ...
    number = Column(Integer, nullable=False)            # 0..999 / 0..9999 según digits
    ...
    __table_args__ = (
        UniqueConstraint('game_id', 'number', name='uq_game_number_per_game'),
    )
